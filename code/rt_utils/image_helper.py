...
def load_sorted_image_series_list(dicom_series_list: list):
    series_data = []

    for dicom_series_path in dicom_series_list:
        try:
...
def get_patient_to_pixel_transformation_matrix(series_data):
    # 返回变换矩阵，用来将ContourData中的真实世界坐标（有着实际距离含义的）变换成像素点位置，便于后续各种指标的计算
    # 变换矩阵举例如下
    # [[1.0240000e+00  0.0000000e+00  0.0000000e+00  2.5600000e+02]
    # [0.0000000e+00  1.0240000e+00  0.0000000e+00  1.8298882e+02]
    # [0.0000000e+00  0.0000000e+00  2.0000000e-01 - 4.9804001e+00]
    #  [0.0000000e+00  0.0000000e+00  0.0000000e+00  1.0000000e+00]]
    # 第一行代表对x坐标的缩放与偏置
    # 第二行代表对y坐标的缩放与偏置
    # 第三行代表对z坐标的缩放与偏置
    # 具体用法见调用这个函数的实际应用，是在三维点之后补一个“1”，然后通过矩阵计算完成三维变换（其实在本应用中只是缩放）+偏置的影响
    first_slice = series_data[0]

    offset = np.array(first_slice.ImagePositionPatient) #第一张图的患者坐标系参考坐标
    row_spacing, column_spacing = first_slice.PixelSpacing #患者坐标系一个点意味着实际当中的长度，单位是毫米
    slice_spacing = get_spacing_between_slices(series_data) #切片之间的间隔，从实现来看假设是等间隔的
    row_direction, column_direction, slice_direction = get_slice_directions(first_slice)
...
def get_slice_position(series_slice: Dataset):
    # 按照切片的排列方向（对于琦琦的case，就是按照z轴）进行排序
...
def create_series_mask_from_contour_sequence(series_data, contour_sequence: Sequence):
    ...
    for i, series_slice in enumerate(series_data):
        # 遍历CT图像，找到对应的勾画区域，CT图像在z轴上是不重叠的，但对应的勾画图像可能是重叠的
        slice_contour_data = get_slice_contour_data(series_slice, contour_sequence)
        #print(len(slice_contour_data))
        ...
        instanceUIDs.append(series_slice.SOPInstanceUID)
    return mask, instanceUIDs


def get_slice_contour_data(series_slice: Dataset, contour_sequence: Sequence):
    # 通过SOPInstanceUID的匹配找到CT图像与勾画图像的对应，返回对应的勾画图像（只返回了三维坐标点），丢弃了其它信息，它们的坐标才是可比的
    slice_contour_data = []
    ...
    for contour in contour_sequence:
        for contour_image in contour.ContourImageSequence:
            if contour_image.ReferencedSOPInstanceUID == series_slice.SOPInstanceUID:
                ...
                instanceUID = contour_image.ReferencedSOPInstanceUID
...
def get_slice_mask_from_slice_contour_data(
    ...
    for contour_coords in slice_contour_data:
        reshaped_contour_data = np.reshape(contour_coords, [len(contour_coords) // 3, 3]) #将列表折叠成三维点坐标的二维列表
        translated_contour_data = apply_transformation_to_3d_points(reshaped_contour_data, transformation_matrix) #计算变换之后的点坐标
        polygon = [np.around([translated_contour_data[:, :2]]).astype(np.int32)] #将坐标四舍五入并取出前两个（x、y坐标）
        polygon = np.array(polygon).squeeze()
        polygons.append(polygon)
    slice_mask = create_empty_slice_mask(series_slice).astype(np.uint8)
    # 加一组轮廓mask图，用来计算Hausdorff距离，WIP

    cv.fillPoly(img=slice_mask, pts = polygons, color = 1)
    '''
    if len(slice_contour_data) > 1:
        print(sum(slice_mask.astype(np.uint32)))
        import random
        name = str(random.randint(1, 100))
        cv.imwrite(f'/Users/bytedance/project/code/{name}.jpg', slice_mask)
    '''

    return slice_mask

def create_empty_series_mask(series_data):
    # 创建一张mask图，长宽是记录在切片中的图像长宽，这里用了第一个切片的记录，实际上同一组CT的长宽应该是一样的
    # 高度是总的CT图片数量，也即认为每个图片的高度是1，并且是等间距的
    # 遗留问题：我们后面要看看是不是CT图片的z轴坐标都是唯一且等间距的
