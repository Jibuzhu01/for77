...
class RTStruct:
    ...
    def get_roi_mask_by_name(self, name) -> np.ndarray:
        ...
        contour_sequence是一个子对象，有四个属性，其中点坐标记录在ContourData中，是连续的浮点数表示的三维坐标，可以通过reshape进行空间恢复
        series_data是image_helper中对原始CT图像路径调用load_sorted_image_series的结果，这个方法将原始ct图像（不包含标注的RS图像）按照z轴坐标进行了排序
