...
def output(name, roi1, IncI, CI, RVD, mean_surface_distance, centroid_distance, dice_coefficient, hausdorff_distance_95):
    fr = open("/Users/guqiqi/Downloads/coderesult/Aprilcase5.csv", 'a')
...
if __name__ == "__main__":
    student_path = "/Users/guqiqi/Downloads/case5"
    tea_path = "/Users/guqiqi/Downloads/case5/PUMCH05-Standard"
    for _, tmp_student_path, _ in os.walk(student_path):
        for tmp in tmp_student_path:
            want_path = student_path + "/" + tmp
            print(want_path)
            main(want_path, tea_path, roi1="CTV_PRE", roi2="CTV_SD", vision=False)
            if "ZhangWenyan" in want_path:
                main(want_path, tea_path, roi1="CTV_POST", roi2="CTV_SD", vision=False)
            if "TangRuiming" in want_path:
                main(want_path, tea_path, roi1="CTV_POST", roi2="CTV_SD", vision=False)
            if "CuiZhiyong" in want_path:
                main(want_path, tea_path, roi1="CTV_POST", roi2="CTV_SD", vision=False)
            if "ZhangPing" in want_path:
                main(want_path, tea_path, roi1="CTV_POST", roi2="CTV_SD", vision=False)
                main(want_path, tea_path, roi1="CTV_PRE2", roi2="CTV_SD", vision=False)
                main(want_path, tea_path, roi1="CTV_POST2", roi2="CTV_SD", vision=False)
            if "JiangYingxiao" in want_path:
                main(want_path, tea_path, roi1="CTV_POST", roi2="CTV_SD", vision=False)
            if "ChengZhiwen" in want_path:
                main(want_path, tea_path, roi1="CTV_POST", roi2="CTV_SD", vision=False)
